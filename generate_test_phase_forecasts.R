# Generate Test Phase Forecasts for Hub Submission
...
print(paste("Generating test phase forecasts for", length(test_origin_dates), "origin dates"))
...
# Function to generate VAR forecast for one origin date
generate_var_forecast <- function(data, forecast_date, quantile_levels, model_id) {
  
  # Filter training data
  train_data <- data |>
    filter(origin_date <= forecast_date)
  
  # Fit VAR model with sqrt transformation
...
for (i in seq_along(test_origin_dates)) {
  ...
  tryCatch({
    forecast_result <- generate_var_forecast(
      flu_data_wide,
      as.Date(origin_date),
      quantile_levels,
      "sismid-var2-sqrt"
...
this_model_id <- "sismid-var2-sqrt"
...
metadata_text <- c(
  ...
  "methods: \"VAR(2) model with square root transformation. Captures cross-regional dependencies in flu transmission. All 11 time series (10 HHS regions + national) modeled jointly. Square root transformation stabilizes variance. Two-week lag structure (AR2) captures autocorrelation. Bootstrap method used for prediction intervals.\"",
...
summary_text <- paste0(
  ...
  "## Next Steps:\n",
  "1. Fork the hub repository on GitHub\n",
  "2. Clone your fork locally\n",
  "3. Copy forecast files to your fork\n",
  "4. Commit and push\n",
  "5. Create pull request\n"
)

writeLines(summary_text, "test_phase_summary.txt")
print("\nSubmission summary saved to: test_phase_summary.txt")