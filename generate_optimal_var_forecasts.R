# Generate Test Phase Forecasts for Optimal VAR Model
# Final model: VAR(1) + season(52) with sqrt transformation
...
print(paste("Generating optimal VAR(1) + season(52) forecasts for", length(test_origin_dates), "origin dates"))
...
# Function to generate optimal VAR forecast for one origin date
generate_optimal_var_forecast <- function(data, forecast_date, quantile_levels, model_id) {
  
  # Filter training data
  train_data <- data |>
    filter(origin_date <= forecast_date)
  
  # Fit optimal VAR model: VAR(1) + season(52) with sqrt transformation
...
for (i in seq_along(test_origin_dates)) {
  ...
  tryCatch({
    forecast_result <- generate_optimal_var_forecast(
      flu_data_wide,
      as.Date(origin_date),
      quantile_levels,
      "sismid-var1-optimal"
...
this_model_id <- "sismid-var1-optimal"
...
metadata_text <- c(
  "team_abbr: \"sismid\"",
  "model_abbr: \"var1-optimal\"",
  "model_name: \"Optimal Seasonal Vector Autoregression\"",
  "designated_model: true",
  "methods: \"VAR(1) model with square root transformation and annual seasonality (period=52). Optimized through extensive model comparison. Captures cross-regional dependencies with minimal lag structure, relying on seasonal component for temporal patterns. All 11 time series (10 HHS regions + national) modeled jointly. Square root transformation stabilizes variance. One-week lag structure (AR1) with annual seasonality captures yearly flu patterns efficiently. Bootstrap method used for prediction intervals. AICc: -49,358, substantially better than VAR(2) alternatives.\"",
...
writeLines(summary_text, "OPTIMAL_VAR_SUBMISSION.md")
print("\nFinal submission summary saved to: OPTIMAL_VAR_SUBMISSION.md")
print("\n🎯 READY FOR HUB SUBMISSION! 🎯")